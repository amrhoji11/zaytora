...
// middle into two equal panels, each hinged on its own outer edge (left
// panel on the left edge, right panel on the right edge) and swings open
// outward in 3D — not tied to the seal position at all, unlike every other
// envelope-cover variant, because a real door's split is dictated by its own
// frame, not by where a wax seal happens to sit. See DoorSlideEnvelopeCover
// for the other door variant, which pulls each panel straight out sideways
// instead of hinging it open.
const OPEN_DURATION_MS = 1800;
const HOLD_DURATION_MS = 500;
const FADE_DURATION_MS = 700;
// The two panels are a matched pair — they swing open together, not one
// after the other (a real door doesn't wait for one leaf before the second
// moves), so the stagger between them is only enough to avoid two perfectly
// identical transitions animating as one flat plane.
...
export function DoorFoldEnvelopeCover({
  onOpen,
  standalone,
  imageSrc,
  imageAlt,
  ...
  return (
    ...
    >
      {/* A faint forward dolly as the doors swing — a real camera doesn't
          stay perfectly still while walking through a doorway, and that
          small push-in is what reads as "stepping through" rather than
          "watching two panels rotate in place". */}
      <motion.div
        className="relative mx-auto h-full w-full max-w-[393px]"
        style={{ perspective: 1400 }}
        animate={opening ? { scale: 1.06 } : { scale: 1 }}
        transition={{ duration: OPEN_DURATION_MS / 1000, ease: [0.4, 0, 0.2, 1] }}
      >
        {/* Left leaf — hinged on the left edge, swings back and away around
            a vertical axis, exactly like pushing open a real door from the
            right-hand side of its own frame. */}
        <motion.div
          ...
          animate={opening ? { rotateY: -112, opacity: 0.55 } : { rotateY: 0, opacity: 1 }}
        ...
        {/* Right leaf — hinged on the right edge, swings open a beat behind
            the left one so the pair doesn't read as a single flat plane
            splitting in two. */}
        <motion.div
          className="absolute inset-0"
          style={{
        ...
        {/* Bright light spilling through the center seam as the two leaves
            swing apart — a vertical bar widening from the middle, standing
