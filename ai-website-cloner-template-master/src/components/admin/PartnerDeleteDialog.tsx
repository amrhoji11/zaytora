...
const COPY = {
  ar: {
    title: "هل أنت تأكد من حذف هذا الشريك؟",
    body: (name: string) => `سيتم حذف "${name}" نهائياً وإخفاء كود الشريك الخاص به فوراً من صفحة شركاؤنا. لا يمكن التراجع عن هذا الإجراء.`,
    delete: "حذف",
    cancel: "إلغاء",
  },
  en: {
    ...
    body: (name: string) =>
      `"${name}" will be permanently removed and their promo code hidden from /OurPartners immediately. This action cannot be undone.`,
    delete: "Delete",
    cancel: "Cancel",
  },
};
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <p className="mt-2 text-sm leading-relaxed text-body-foreground">{t.body(businessName)}</p>
